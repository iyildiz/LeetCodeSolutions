...
public class FirstNonRepeatingCharacter {
  public int firstNonRepeatingCharacter1(String input) {
    ...
    for (int i = 0; i < input.length(); i++) {
      ...
      existingCharacterOccurrences.put(
          ch,
          numberOfOccurrence == null
              ? new CharacterOccurrence(i, 1)
              : new CharacterOccurrence(i, numberOfOccurrence.occurrence + 1));
    }

    var firstNonRepeatingCharacter =
        existingCharacterOccurrences.entrySet().stream()
            .filter(entry -> entry.getValue().occurrence == 1)
            .findFirst();

    return firstNonRepeatingCharacter.map(entry -> entry.getValue().firstIndex).orElse(-1);
  }
